...
{
    private readonly Dictionary<int, IComponent> _activeComponents = new();
    private readonly List<IComponent> _children = new();
    private readonly List<IComponent> _childrenRenderOrder = new();

    private readonly Stack<int> _idStack = new();
    private readonly Dictionary<int, int> _idsUsedThisFrame = new();
    private readonly Queue<Action> _nextTick0 = new();
    private readonly Queue<Action> _nextTick1 = new();

    private readonly Stack<(IParent Parent, int MaxChildren, int ChildrenCount)> _parents = new();
    private readonly Queue<(int Id, IParent Parent, IComponent Component)> _pendingComponents = new();
    ...
    private int? Focus
    {
        ...
        set
        {
            if (_focus != null) _activeComponents[_focus.Value].IsFocused = false;
            _focus = value;
            if (_focus != null) _activeComponents[_focus.Value].IsFocused = true;
        }
    }
    ...
    {
        if (_activeComponents.TryGetValue(id, out c)) return true;
    ...
    public IParent GrabParent(IComponent c)
    {
        ...
        if (c.Parent != current) _pendingComponents.Enqueue((c.Id, _currentParent, c));

        if (_maxChildren > 0 && ++_childrenCount >= _maxChildren) Pop();
    ...
    {
        if (!isAbsoluteId) id = CombineHash(_idHash, id);

        if (_idsUsedThisFrame.TryGetValue(id, out var count))
        {
            count++;
    ...
    private IComponent ExtractPrev(int id)
    {
        return _activeComponents[id].GetPrev();
    }

    private IComponent ExtractNext(int id)
    {
        return _activeComponents[id].GetNext();
    }

    private void FindFocus(Func<int, IComponent> getNeighbor)
    {
        var initialFocus = Id;
        if (Focus != null) initialFocus = Focus.Value;
        var newFocus = initialFocus;
        do
        {
    ...
    private void ComputeIdStack()
    {
        unchecked
        {
            ...
            foreach (var e in _idStack) hash *= 31 + e.GetHashCode();
