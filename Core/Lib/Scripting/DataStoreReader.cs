...
public class DataStoreReader
{
    ...
    public object Get(string key)
    {
        ...
        if (data is VariableWrapper wrapper) return wrapper.Get();
        return data;
    }
    ...
    {
        if (_dataStore.GetData(key) is not VariableWrapper wrapper) throw new Exception("Not a function");

        if (wrapper.Get() is not LuaFunction function) throw new Exception("Not a function");
