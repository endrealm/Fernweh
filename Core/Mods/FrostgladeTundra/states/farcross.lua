...
StateBuilder("enter_market")
        ...
        function(renderer, context)
            ...
            if (IsQuestDiscovered("mugging_quest") == false) then
                renderer:AddAction(function()
                    context:ChangeState("mugging_quest")
                end, "button.market.crowd")
            end
            renderer:AddAction(function()
                context:Exit()
            end, "button.leave")
...
StateBuilder("enter_tavern")
        ...
        function(renderer, context)
            ...
            if (IsQuestCompleted("mugging_quest") == false) then
                ...
                renderer:AddAction(function()
                    context:StartBattle({ "bandit" }, "cave", "beat_mugger")
                end, "button.battle")
            end

            if (IsInParty("Borof") == false) then
                renderer:AddAction(function()
                    context:ChangeState("recruit_borok")
                end, "button.tavern.borof")
            end
            renderer:AddAction(function()
                context:ChangeState("enter_inn")
            end, "button.leave")
        end
)
        :Build()
...
StateBuilder("enter_hall")
        ...
        function(renderer, context)
            ...
            then
                renderer:AddAction(function()
                    context:ChangeState("dolrom_quest1")
                end, "button.checkquest.dolrom")
            elseif (HasJustCompletedQuest("rat_cellar"))
            then
                renderer:AddAction(function()
                    context:ChangeState("dolrom_quest2")
                end, "button.checkquest.dolrom")
            end
